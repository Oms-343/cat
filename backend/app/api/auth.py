...
def login(payload: LoginRequest, session: SessionDep) -> LoginResponse:
    ...
    if user.role in (UserRole.ADMIN, UserRole.SUPER):

        _validate_official_email(email)

    elif user.role == UserRole.MSME:

        raise HTTPException(

            status_code=status.HTTP_403_FORBIDDEN,

            detail="MSME accounts use WhatsApp enrollment links to update their profile",
        ...
        raise HTTPException(
            ...
            detail="account type cannot sign in",
